...
const añadirServicio = document.getElementById('servicioAñadir')
...
document.getElementById("botonAñadir").addEventListener("click", function (event) {
    ...
    let servicioAñadir = añadirServicio.value
    let imagenAñadir = añadirImagen.value

    let van = true

    if (productoAñadir == '' || valorAñadir == '' || servicioAñadir == '' || imagenAñadir == '') {
...
window.addEventListener("load", () => {
    ...
    for (let i = 0; i < productos.length; i++) {
        mostraProductos.innerHTML += `<div class="contenedorProductos"><img src="${productos[i].urlImagen}"><div class="informacion"><p>${productos[i].nombre}</p><p class="precio"><span>Precio: ${productos[i].valor}$</span></p> Servicio: ${productos[i].servicio}<p></p></div></div>`
    }
})
